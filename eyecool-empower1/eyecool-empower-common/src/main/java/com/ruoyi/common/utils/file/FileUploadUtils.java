...
public class FileUploadUtils {
     ...
     * imgBase64 base6流
     * path 保存后的路径
     * imgFullName：保存后的名称 包含后缀名 如zfx.jpg
     **/
    public static boolean generateImage(String imgBase64, String path, String imgFullName) {
        ...
        try {
            File desc = getAbsoluteFile(path, imgFullName);
            FileUtils.writeByteArrayToFile(desc, Base64.decodeBase64(imgBase64));
            return true;
        } catch (IOException e) {
            e.printStackTrace();
            return false;
