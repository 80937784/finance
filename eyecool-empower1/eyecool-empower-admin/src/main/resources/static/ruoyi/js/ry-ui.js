...
(function ($) {
    $.extend({
    	...
    	table: {
    		...
    		// 导出数据--mawenjun自定义20191025
    		exportExcelWithMultiSelect: function(formId) {
    			table.set();
    			$.modal.confirm("确定导出所有" + table.options.modalName + "吗？", function() {
	    			var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
				var params = $("#" + table.options.id).bootstrapTable('getOptions');
	    			var arr = $("#" + currentId).serializeArray();
				arr.push({ "name": "orderByColumn", "value": params.sortName });
	    			arr.push({ "name": "isAsc", "value": params.sortOrder });
	    			var rows = $.table.selectColumns("id");
					arr.push({name:"ids", value:rows});
	    			$.modal.loading("正在导出数据，请稍后...");
	    			$.post(table.options.exportUrl, arr, function(result) {
	    				if (result.code == web_status.SUCCESS) {
	    			        window.location.href = ctx + "common/download?fileName=" + encodeURI(result.msg) + "&delete=" + true;
	    				} else if (result.code == web_status.WARNING) {
	                        $.modal.alertWarning(result.msg)
    		...
    		// 下载图片--mawenjun自定义20191025
    		downloadImgsWithMultiSelect: function(formId, extendArr) {
    			table.set();
				var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
    			var arr = $("#" + currentId).serializeArray();
    			var rows = $.table.selectColumns("id");
				arr.push({name:"ids", value:rows});
				if(extendArr && extendArr.length){
					arr.push(...extendArr);
					arr = Array.from(new Set(arr))
				}
    			$.modal.loading("正在下载，请稍后...");
    			$.post(table.options.downloadUrl, arr, function(result) {
    				if (result.code == web_status.SUCCESS) {
    			        window.location.href = ctx + "common/download?fileName=" + encodeURI(result.msg) + "&delete=" + true;
    				} else if (result.code == web_status.WARNING) {
                        $.modal.alertWarning(result.msg)
    		...
    		// 下载图片--mawenjun自定义20191025
    		downloadImgsWithConfirm: function(){
    			$.modal.confirm("确定下载所有" + table.options.modalName + "图片吗？", function() {
	    			table.downloadImgsWithMultiSelect();
    			});
    		},
    		// 下载模板
    	...
    	modal: {
            ...
            // 弹出层指定宽度, 只有关闭按钮--mawenjun自定义20191028
            openOnlyWithClose: function (title, url, width, height) {
            	//如果是移动端，就使用自适应大小弹窗
            	if ($.common.isMobile()) {
            	    width = 'auto';
            	    height = 'auto';
